...
drill_mode_completion(char* string, yed_completion_results* results)
{
    ...
    FN_BODY_FOR_COMPLETE_FROM_ARRAY(
        string, 2, mode_strs_lowercase, results, status);
...
yed_plugin_boot(yed_plugin* self)
{
    ...
    yed_plugin_set_completion(
        Self, "drill-bind-compl-arg-0", drill_mode_completion);
    yed_plugin_set_completion(
        Self, "drill-bind-compl-arg-2", yed_get_completion("command"));
    yed_plugin_set_completion(
        Self, "drill-unbind-compl-arg-0", drill_mode_completion);
...
drill_repeat(void)
{
    ...
    repeating = 1;
    array_traverse(repeat_keys, key_it) { _drill_take_key(*key_it, NULL); }
    repeating = 0;
}
